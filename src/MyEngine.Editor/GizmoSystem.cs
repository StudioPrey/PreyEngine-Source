...
public sealed class GizmoSystem
{
    ...
    /// <summary>Call once per frame after the Viewport panel has been drawn (uses the foreground draw list, so window order doesn't matter).</summary>
    public void Update(EditorState state, XnaMatrix viewMatrix, float zoom, ImVec2 imageMin, bool viewportHovered)
    {
