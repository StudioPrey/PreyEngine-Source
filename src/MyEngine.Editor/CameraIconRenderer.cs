...
public sealed class CameraIconRenderer : IDisposable
{
    ...
    /// to render the scene and the Viewport image's screen rect (icons are clipped to stay inside it, so
    /// a camera near the edge doesn't visually bleed onto the Hierarchy/Inspector panels).</summary>
    public void Draw(EditorState state, XnaMatrix viewMatrix, ImVec2 imageMin, ImVec2 imageMax)
    {
