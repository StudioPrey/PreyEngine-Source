...
/// <summary>
/// Computes the Editor's fixed panel arrangement (Hierarchy left, Inspector right, the combined Console/
/// Content Browser panel along the bottom, Viewport filling the rest) every frame from the current window
...
public static class EditorLayout
{
    ...
    private static Vector2 _bottomPanelPos, _bottomPanelSize;
    ...
    public static void Refresh()
    {
        ...
        // Console and Content Browser now share this one rectangle as tabs (see Panels/BottomPanel.cs)
        // instead of splitting it side by side — each gets the full width when it's the active tab.
        _bottomPanelPos = new Vector2(bodyPos.X + hierarchyWidth, bodyPos.Y + topHeight);
        _bottomPanelSize = new Vector2(centerWidth, bottomHeight);
    }
    ...
    public static void PinBottomPanel() => Pin(_bottomPanelPos, _bottomPanelSize);
