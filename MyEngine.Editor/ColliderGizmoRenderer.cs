...
/// rectangle, a circle as a ring — on the Viewport window's own draw list (handed in from
/// ViewportPanel.Draw's result), the same technique GizmoSystem and CameraIconRenderer already use.
/// Purely visual: this never touches simulation state.
///
...
public static class ColliderGizmoRenderer
{
    ...
    /// used to render the scene (EditorApp._currentViewMatrix / _currentZoom), the Viewport image's screen
    /// rect (outlines are clipped to stay inside it), and the Viewport window's own draw list
    /// (ViewportResult.DrawList) so this stays part of that window's normal paint order instead of
    /// bleeding on top of unrelated popups/modals.</summary>
    public static void Draw(EditorState state, XnaMatrix viewMatrix, float zoom, ImVec2 imageMin, ImVec2 imageMax, ImDrawListPtr drawList)
    {
