...
public static class EditorIcons
{
    ...
    public static bool Button(string id, Vector2 size, bool active, Action<ImDrawListPtr, Vector2, Vector2, uint> drawIcon)
    {
        ...
        // GetWindowDrawList() — NOT GetForegroundDrawList(). The foreground list paints above literally
        // every window and popup in the whole application, with no regard for normal z-order; that's what
        // was making these icons render on top of the File/Edit/Scripts dropdown menus instead of behind
        // them. The window draw list keeps the icon in the same paint order as everything else in the
        // window it's drawn from (right after the button background, still part of that window's normal
        // stacking), so a popup opened above this window correctly covers it again.
    ...
    /// <summary>A speaker cone (box + flared trapezoid, the trapezoid split into two triangles since
    /// AddQuadFilled isn't in this file's proven-primitive set) plus two sound-wave arcs approximated the
    /// same way CurvedArrow approximates its sweep — a handful of AddLine segments along the arc, rather
    /// than PathArcTo. Used both for AssetType.Audio tiles in the Content Browser and, at a larger size,
    /// for AudioIconRenderer's Viewport overlay over any GameObject with an AudioSource.</summary>
    public static void Audio(ImDrawListPtr dl, Vector2 min, Vector2 max, uint color)
    {
        float w = max.X - min.X;
        float h = max.Y - min.Y;
        Vector2 P(float nx, float ny) => new(min.X + nx * w, min.Y + ny * h);

        dl.AddRectFilled(P(0.14f, 0.38f), P(0.40f, 0.62f), color);

        // flared cone: quad A-B-C-D split into triangles ABC and ACD
        Vector2 a = P(0.40f, 0.38f), b = P(0.66f, 0.14f), c = P(0.66f, 0.86f), d = P(0.40f, 0.62f);
        dl.AddTriangleFilled(a, b, c, color);
        dl.AddTriangleFilled(a, c, d, color);

        Vector2 waveCenter = P(0.62f, 0.5f);
        Arc(dl, waveCenter, 0.16f * w, -0.55f, 0.55f, color, 1.4f);
        Arc(dl, waveCenter, 0.27f * w, -0.55f, 0.55f, color, 1.4f);
    }

    /// <summary>A handful of AddLine segments along a circular arc — the same technique CurvedArrow above
    /// uses for its sweep, minus the arrowhead. Kept to this file's proven-primitive policy rather than
    /// reaching for PathArcTo/PathStroke.</summary>
    private static void Arc(ImDrawListPtr dl, Vector2 center, float radius, float startAngle, float endAngle, uint color, float thickness)
    {
